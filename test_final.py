"""Final test: write correct agent.py, fix nodes.py, then run."""
import sys

# ---- Patch nodes.py (context_monitor_route + verifier_node) ----
...
ok1 = 'attempts >= max_attempts' in nc
ok2 = 'planner" if not passed' in nc
print(f"nodes.py: max_attempts={ok1}, ctx_next={ok2}")

# ---- Write complete correct agent.py ----
agent = r'''"""Graph Agent —— 基于 MultiAgent 工作流的事件流.
...
print(f"agent.py written: {len(agent)} bytes")
...
from mokioclaw.graph.nodes import context_monitor_route, verifier_node
import inspect

print(f"stream_agent_events uses _stream_workflow: {'_stream_workflow_events' in inspect.getsource(stream_agent_events)}")
print(f"context_monitor_route max_attempts: {'attempts >=' in inspect.getsource(context_monitor_route)}")
print(f"verifier_node ctx_next: {'context_next_node' in inspect.getsource(verifier_node)}")

# Run test
from mokioclaw.cli.app import app
from typer.testing import CliRunner

print("\n=== Running mokioclaw 'hi' ===")
runner = CliRunner()
